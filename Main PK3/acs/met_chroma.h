script 474 (int which, int transferVelocity, int count) clientside
{
    ...
    {
        int partTID = UniqueTID(12000, 14000);
...
script 475 (int which, int a1, int a2)
{
    ...
    switch (which)
    {
      case 0:
        ...
        if (CheckInventory("IceBeamChilled")) { firetime = oldceil(firetime * 1.25); }
      ...
      case 3:
        ...
        ACS_ExecuteAlways(484, 0, packShorts(oldround(x), oldround(nx)),
                                  packShorts(oldround(y), oldround(ny)),
                                  packShorts(oldround(z), oldround(nz)));
...
script 487 (void) clientside
{
    ...
    int magI = oldmin(ftoi(mag), 360); //1024 This controls how far the beams actually go.
    ...
    int tid       = UniqueTID(8000, 13000);
    int tid_start = UniqueTID(8001, 13000);
    ...
    for (i = 0; i < magI; i += density)
    {
        ...
        alpha = oldmax(0, LASER_NOEFFECTS_FADELENGTH - i)
              + oldmax(0, (i + LASER_NOEFFECTS_FADELENGTH) - magI);

        alpha = oldmin(alpha, LASER_NOEFFECTS_FADELENGTH);

        alpha = (alpha * alpha) / LASER_NOEFFECTS_FADELENGTH;
        alpha = itof(alpha) / LASER_NOEFFECTS_FADELENGTH;

        alpha = FixedMul(oldmin(1.0, 0.125 * density), alpha);
    ...
    {
        SetActorVelocity(tid, vx1, vy1, vz1, 0, 0);
        SetActorProperty(tid, APROP_Alpha, oldmin(1.0, FixedMul(LASER_BASEALPHA, 0.125 * density)));
        Thing_ChangeTID(tid, 0);
    }
}
...
script 489 (int x, int y, int z)
{
    ...
    vx = FixedDiv(vx, mag); vy = FixedDiv(vy, mag); vz = FixedDiv(vz, mag);
    magI = oldmin(ftoi(mag), 1024); //This controls the actual damage puff distance.
