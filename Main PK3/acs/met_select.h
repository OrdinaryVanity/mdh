...
#define SM_HUDX             1024		// [R4L] can change for higher res usage
#define SM_HUDY             768			// [R4L] can change for higher res usage
...
script METROID_SELECT_CLIENT (int onOff) clientside
{
    ...
    int maxlen = FixedMul(oldmin(SM_HUDX, SM_HUDY), 0.35);
    ...
    if (actualBeams > 1)
    {
        ...
        {
            FadeTo(0, 0, 0, oldmin(0.35, 0.05 * (time + 1)), 0);
			
			int vx  = -GetPlayerInput(-1, INPUT_YAW) * SM_BASEMOUSERES;
			int vy  = -GetPlayerInput(-1, INPUT_PITCH) * SM_BASEMOUSERES;
			
			//[R4L] check if player has Invert Mouse CVar on, reverse mouse coords if so.
			if(GetCVar("invertmouse") == 1){
				vy  = GetPlayerInput(-1, INPUT_PITCH) * SM_BASEMOUSERES;
            ...
            if (centerdist >= SM_MINCENTERDIST)
            {
                ...
                else { whichSpoke = oldceil(whichSpoke); }
            ...
            highlight_beam2 = 0;
            spokelength = oldmin(spokelength + 32, maxlen);
            time++;
        }
