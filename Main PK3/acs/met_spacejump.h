...
{
    int i = oldmax(playerTimers[pln][timerNum]-1, 0);
...
script METROID_WALLBOUNCE (int type, int direction, int mask)
{
    ...
    if (oldabs(x) > oldabs(y))
    {
        y = FixedMul(y, FixedDiv(20.0, oldabs(x)));
        x = 20.0 * sign(x);
    }
    else
    {
        x = FixedMul(x, FixedDiv(20.0, oldabs(y)));
        y = 20.0 * sign(y);
    }

    canBounce = 0;
    tid = UniqueTID(25000, 30000);

    for (i = 0; i < 10; i++)
    {
        x3 = GetActorX(0) + x + (x2*i);
        y3 = GetActorY(0) + y + (y2*i);
        z  = GetActorZ(0) + 16.0; //16.0
...
script METROID_SPACEJUMP_ENTER enter
{
    ...
    while (PlayerInGame(pln))
    {
        ...
        if (!CheckInventory("CanSpaceJump"))
        {
            ...
            {
                playerJumps[pln] = oldmax(1, playerJumps[pln]);
            }
            Delay(1);
            continue;
        }
        ...
        DidSpecials[pln] = oldmax(0, DidSpecials[pln]-1);

        Delay(1);
    }
}

script METROID_SPACEJUMP_ENTER2 enter
{
    int pln = PlayerNumber();
    ...
    while (ClientEnterLocks[pln] == myLock)
    {
        ...
        else { wasGround = oldmax(0, wasGround-1); }
        ...
        if (!(GetActorVelX(0) || GetActorVelY(0))) //change pln to 0 if not working
        {
