...
// They come in handy :>
// [R4L] Yes, handy indeed. Unfortunately, these are superceded in GZDoom. I've renamed
...
function int oldmin(int x, int y)
{
    if (x < y) { return x; }
    return y;
}

function int oldmax(int x, int y)
{
    if (x > y) { return x; }
    return y;
}
...
{
    if ((x < z) && (y < z)) { return oldmax(x, y); }
    return oldmax(oldmin(x, y), z);
}
...
{
    int s1 = oldsqrt(FixedMul(b, b)-(4*FixedMul(a, c)));
    int s2 = (2 * a);
...
function int sliceString(int string, int start, int end)
{
    ...
    start = oldmax(0, start);
    end   = oldmin(end, len-1);
...
function int oldstrcmp(int str1, int str2)
{
    int i,j,k,l;
...
function int unusedTID(int start, int end) //This is what UniqueTID is for :P
{
    int ret = start - 1;
    int tidNum;
...
function int giveHealthMax(int amount, int maxHP)
{
    ...
    if (maxHP == 0) { newHP = oldmax(curHP, curHP+amount); }
    else
    {
...
function void GiveAmmo(int type, int amount)
{
    if (GetCVar("sv_doubleammo"))
    {
        ...
        int expected = oldmin(m, CheckInventory(type) + amount);
...
function void GiveActorAmmo(int tid, int type, int amount)
{
    ...
    {
        int m = GetAmmoCapacity(type);
        int expected = oldmin(m, CheckActorInventory(tid, type) + amount);
...
function int quadSlope(int orgX, int orgY, int pntX, int pntY, int floatY)
{
    ...
    int slope = cond(floatY, oldsqrt(height), oldsqrt(itof(height)));
